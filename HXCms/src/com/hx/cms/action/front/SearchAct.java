...
public class SearchAct {
	...
	public static final String SEARCH_JYS = "tpl.searchJYS";
	public static final String SEARCH_JYSDB = "tpl.searchJYSDB";
	public static final String SEARCH_JGJG = "tpl.searchJGJG";
	public static final String SEARCH_CJSJ = "tpl.searchCJSJ";
	...
	@RequestMapping(value = "/searchJYS*.jspx", method = RequestMethod.GET)
	public String searchJYS(HttpServletRequest request,
			HttpServletResponse response, ModelMap model) {		
			CmsSite site = CmsUtils.getSite(request);
			// 将request中所有参数保存至model中。
			model.putAll(RequestUtils.getQueryParams(request));
			...
			model.addAttribute("q",parseQ);
				return FrontUtils.getTplPath(request, site.getSolutionPath(),
						TPLDIR_SPECIAL, SEARCH_JYS);
			}
	
	@RequestMapping(value = "/searchJYSDB*.jspx", method = RequestMethod.GET)
	public String searchJYSDB(HttpServletRequest request,
			HttpServletResponse response, ModelMap model,Integer contentId) {		
			CmsSite site = CmsUtils.getSite(request);
			// 将request中所有参数保存至model中。
			model.putAll(RequestUtils.getQueryParams(request));
			FrontUtils.frontData(request, model, site);
			...
			model.addAttribute("q",parseQ);
				return FrontUtils.getTplPath(request, site.getSolutionPath(),
						TPLDIR_SPECIAL, SEARCH_JYSDB);
			}
	
	@RequestMapping(value = "/searchJGJG*.jspx", method = RequestMethod.GET)
	public String searchJGJG(HttpServletRequest request,
			HttpServletResponse response, ModelMap model) {		
		CmsSite site = CmsUtils.getSite(request);
		// 将request中所有参数保存至model中。
		model.putAll(RequestUtils.getQueryParams(request));
	...
	@Autowired
	private ChannelMng channelMngImpl;

package com.hx.cms.action.directive.abs;
...
public abstract class AbstractContentDirective implements
	...
	protected boolean getWGZ(Map<String, TemplateModel> params)
			throws TemplateException {
		String wgz = DirectiveUtils.getString("wgz", params);
		if (wgz == null) {
			return false;
		} else {
			return true;
	...
	protected Object getData(Map<String, TemplateModel> params, Environment env)
		...
		List<Content> timeList = null ;
		int count = FrontUtils.getCount(params);
		boolean wgz =getWGZ( params);
		if(wgz){
			 timeList = contentMng.getListTime();
		}
		...
		if (channelIds != null) {
			...
			if (isPage()) {
				...
				if(timeList!=null&&timeList.size()>0){
					...
					for (Content content : timeList) {
						List<Content> list =contentMng.getListByChannelIdsForTag(channelIds,
								typeIds, titleImg, recommend, title, orderBy,
								option, first, count,content.getReleaseDate());

...
import static com.hx.common.util.ParseURLKeyword.getKeyword;
...
import com.hx.cms.manager.assist.CmsSiteFlowMng;
import com.hx.cms.statistic.FlowBean;
import com.hx.common.ipseek.IPSeeker;
import com.hx.common.util.DateFormatUtils;

@Service
public class CmsSiteFlowCacheImpl implements CmsSiteFlowCache, DisposableBean {
	private static final  Logger log = LoggerFactory.getLogger(CmsSiteFlowCacheImpl.class);
	...
	private static String getRefererWebSite(String referer){
		...
		if (start <= 0) {
			log.error("referer website uri not like 'http://.../...' pattern: "+ referer);
		}
		String url = null;
		try {
			url = referer.substring(0, start);
		} catch (Exception e) {
			/**
			 * phz oline test
			 */
			System.out.println("getRefererWebSite--------"+referer);
		}

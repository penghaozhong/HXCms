...
import com.hx.common.hibernate3.HibernateSimpleDao;
import com.hx.common.page.Paginable;
import com.hx.common.page.SimplePage;
...
public class StaticPageDaoImpl extends HibernateSimpleDao implements
		...
		while (channels.next()) {
			...
			if (c.getParentId()==null) {
				...
				if (quantity <= 0) {
					...
					if( ((quantity - 1) / c.getPageSize() + 1)< SimplePage.STATIC_COUNT){
						totalPage = (quantity - 1) / c.getPageSize() + 1;
					}else{
						totalPage= SimplePage.STATIC_COUNT;
					}
	 ...
	 * @author wzw
	 * @time 2014-08-29 17:56:36
	 */
	public boolean contentStatic(Content c, Configuration conf,
			Map<String, Object> data,boolean noNeed) throws IOException, TemplateException {
		// 如果是外部链接或者不生成静态页面，则不生成
		Channel chnl = c.getChannel();
		if (!StringUtils.isBlank(c.getLink()) || !chnl.getStaticContent()) {
			return false;
